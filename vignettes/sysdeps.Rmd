...
This is not a hack, Linux package managers have been designed to automatically determine the run-time dependencies between different system libraries. You should use the same methods when providing binaries for R packages, even if they are not distributed in a `rpm` or `deb` file.
...
For example, if you build against [`libcurl4-openssl-dev`](https://packages.debian.org/bullseye/libcurl4-openssl-dev) then the run-time library is [`libcurl4`](https://packages.debian.org/bullseye/libcurl4).
